...
public class OllamaAdapterTests
{
    ...
    public async Task CreateChatCompletionAsync_MapsOpenAIRequestToOllamaFormat()
    {
        ...
        Assert.Equal("ollama/llama3", result.Model); // Original model name preserved
    }
    ...
    public void StripPrefix_RemovesOllamaPrefix()
    {
        ...
        // Используем рефлексию для доступа к приватному методу или тестируем через публичный API
